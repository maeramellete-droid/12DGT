		...
		while True:
			...
			if new_card and all(new_card):
				try:
					...
					print(cards[card_id])
					easygui.msgbox(f"Card added!\n\n{cards[card_id]}",
									title="Monster Card Game")
				# After adding the card ask if user wants to edi it
					edit_msg = "Do you want to edit this card?"
					edit_choice = easygui.buttonbox(edit_msg,
													title= "Monster Card Game",
													choices= ["Edit",
													"Confirm"])
					if edit_choice == "Edit":
						update_dts = "Update card details:"
						updated_card = easygui.multenterbox(update_dts,
															"Edit card",
															["Name",
															"Strength",
															"Speed", 
						...
						# Update the card
							...
							cards[card_id]["Strength"] = max (1,
							min(int(updated_card[1]),25))
							cards[card_id]["Speed"] = max(1,
							min(int(updated_card[2]), 25))
							cards[card_id]["Stealth"] = max(1,
							min(int(updated_card[3]), 25))
							cards[card_id]["Cunning"] = max(1, 
							min(int(updated_card[4]), 25))
